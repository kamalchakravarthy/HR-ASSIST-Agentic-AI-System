        ...
        if data:
            ...
            for record in hist:
                if isinstance(record, str):
                    # Handle ISO format string dates from apply_leave
                    date_obj = datetime.fromisoformat(record)
                    dates.append(date_obj.strftime("%B %d, %Y"))
                elif isinstance(record, dict):
                    # Handle original dictionary format
                    dates.append(record['leave_date'].strftime("%B %d, %Y"))
                else:
                    # Handle datetime objects directly if any
                    dates.append(record.strftime("%B %d, %Y"))
            return f"Leave history for {employee_id}: {', '.join(dates)}."
        return "Employee ID not found."

if __name__ == "__main__":
    lm = LeaveManager()
    print(lm.get_leave_history("E002"))